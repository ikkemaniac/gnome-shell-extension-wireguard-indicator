...
var Control = GObject.registerClass({
    ...
    startService() {
        let argv = "/bin/sudo /bin/systemctl start wg-quick@wg0.service";
        let [ok, pid] = GLib.spawn_async(null, argv.split(" "), null, GLib.SpawnFlags.SEARCH_PATH, null);
        GLib.spawn_close_pid(pid);
    }

    stopService() {
        let argv = "/bin/sudo /bin/systemctl stop wg-quick@wg0.service";
        let [ok, pid] = GLib.spawn_async(null, argv.split(" "), null, GLib.SpawnFlags.SEARCH_PATH, null);
        GLib.spawn_close_pid(pid);
    }
    ...
    _updateSystemdState() {
        if (this._childSource)
            return;
        let argv = "/bin/sudo /bin/systemctl show -p LoadState -p ActiveState wg-quick@wg0.service";
